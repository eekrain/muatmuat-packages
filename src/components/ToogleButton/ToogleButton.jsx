...
const ToogleButton = ({
  ...
  const styleDisabled =
    disabled & getValue
      ? style.disabled_active_primary
      : disabled & !getValue
        ? style.disabled_inactive_primary
        : "";
  useEffect(() => {
    setValue(value);
  }, [value]);
  ...
  return (
    <div
      className={`${styleDisabled} flex w-fit items-center gap-2`}
      {...props}
    >
      <button
      ...
      {!!(textActive || textInactive) && (
        <span className={`${style.text} select-none capitalize`}>
