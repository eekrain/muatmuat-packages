...
const getActionItems = ({ order, userRole, t }) => {
  const actions = [];
...
const useLoadingTime = (schedule) => {
  ...
  return useMemo(() => {
    ...
    const timeRange = `${formatter.format(startDate)} WIB${schedule.endDate ? ` s/d ${formatter.format(new Date(schedule.endDate))} WIB` : ""}`;
...
const PesananCard = ({ order, userRole, className }) => {
  ...
  const actionItems = getActionItems({ order, userRole, t });
          ...
          <div className="flex">
            <div className="flex w-full max-w-[120px] flex-col gap-2.5">
              <div className="line-clamp-1 flex items-center truncate text-xxs font-semibold">
                ...
                {order.orderStatus === ORDER_STATUS.PERLU_RESPON_PERUBAHAN && (
                  <InfoTooltip
                    icon="/icons/warning20.svg"
                    side="right"
                    appearance={{
                ...
                <Link href={`/cs/daftar-pesanan/${order.id}`} target="_blank">
