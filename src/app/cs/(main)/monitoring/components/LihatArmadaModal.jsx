...
const LihatArmadaModal = ({ isOpen, onClose, orderData }) => {
  ...
  const { data, isLoading } = useGetActiveOrdersFleet(orderData?.orderId);

  // Get assigned vehicles from order data
  const assignedVehicles = data?.Data?.fleetList || [];
  // remove debug log
  ...
  const filteredVehicles = assignedVehicles.filter((vehicle) => {
    ...
    return (
      vehicle.vehicleInfo?.licensePlate.toLowerCase().includes(searchLower) ||
      vehicle.driverInfo?.driverName.toLowerCase().includes(searchLower)
    );
  });
    ...
    <Modal open={isOpen} onOpenChange={onClose}>
      <ModalContent className={cn("w-[600px] bg-white p-0")} type="muatmuat">
        ...
        >
          ...
          >
            {isLoading ? (
              // Loading skeleton: show 3 placeholder rows
              [0, 1, 2].map((i) => (
                <div
                  key={`skeleton-${i}`}
                  className={cn(
                ...
                >
                  <div className="flex h-[52px] w-full items-center gap-2 p-0">
                    ...
                    <div className="flex h-[44px] flex-1 flex-col items-start justify-center gap-2 py-1">
