...
const Page = () => {
  ...
  // State for search and filter for available fleet
  const [searchValue, setSearchValue] = useState("");
  const [selectedOperationalStatus, setSelectedOperationalStatus] = useState(
    []
  );

  // Get available fleet data (mock orderId for now)
  const { data: availableFleetData, isLoading: isAvailableFleetLoading } =
    useGetAvailableFleet("order-uuid-1", {
      search: searchValue,
      operationalStatus:
        selectedOperationalStatus.length > 0
  ...
  } = useMonitoringHandlers(dispatches, state, availableFleetData, addToast);
  ...
  // Convert available fleet vehicles to map markers and apply filters
  const allFleetMarkers =
    availableFleetData?.vehicles?.map((vehicle) => {
      ...
      return {
        ...
        title: `${vehicle.licensePlate} - ${vehicle.driver?.name || "-"}`,
        icon: icon,
        rotation: 0,
        fleet: vehicle,
  ...
  // Apply filters to fleet markers (operational status)
  const fleetMarkers = allFleetMarkers.filter((marker) => {
    if (selectedOperationalStatus.length === 0) return true;
    return selectedOperationalStatus.includes(marker.fleet.operationalStatus);
  });
      ...
      >
        ...
        >
          ...
          <div className="relative flex-1 overflow-hidden rounded-[20px] bg-white shadow-muat transition-all duration-300 ease-in-out">
            ...
            {!isAvailableFleetLoading &&
              availableFleetData?.vehicles?.length > 0 && (
                <MapInterfaceOverlayPilihArmada
                  ...
                  onApplyFilter={(statusArr) =>
                    setSelectedOperationalStatus(statusArr)
                  }
                  ...
                  activeFilters={{
                    truck: selectedOperationalStatus,
                    order: [],
                  }}
                  showPilihArmada={panels.showPilihArmada}
                  fleetLocationsData={availableFleetData}
                  hasData={availableFleetData?.vehicles?.length > 0}
                  fleetData={availableFleetData}
                />
              )}
          </div>
