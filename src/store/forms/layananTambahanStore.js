...
export const useLayananTambahanStore = create((set, get) => ({
  ...
  validateForm: () => {
    ...
    if (!nomorHandphonePenerima) {
      emptyField += 1;
      newErrors.nomorHandphonePenerima = "Nomor handphone penerima wajib diisi";
...
export const useLayananTambahanActions = () => {
  const setField = useLayananTambahanStore((state) => state.setField);
  const validateForm = useLayananTambahanStore((state) => state.validateForm);
