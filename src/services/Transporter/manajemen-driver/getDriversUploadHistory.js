...
export const useGetDriversUploadHistory = (url) => {
  return useSWR(url ? `driversUploadHistory-${url}` : null, () =>
    fetcherDropdownVehicle(url, { arg: null })
  );
};

// New hook for upload history with query parameters
export const useGetDriversUploadHistoryWithParams = (params = {}) => {
  ...
  const url = `/v1/drivers/upload-history${queryString ? `?${queryString}` : ""}`;

  // Create a unique key for SWR that includes all parameters
  const swrKey = `driversUploadHistory-${JSON.stringify(params)}`;
