...
const DriverNonaktif = ({ onPageChange, onPerPageChange }) => {
  ...
  const { data, isLoading } = useGetNonActiveDriversData({
    page: currentPage,
    limit: perPage,
    search: searchValue,
    ...filters,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        <div className="text-xs">
          {row.lastActive
            ? new Date(row.lastActive).toLocaleDateString("id-ID")
            : "-"}
        </div>
      ),
    },
    {
      key: "reason",
      header: "Alasan Nonaktif",
      render: (row) => (
        <div className="text-xs">{row.inactiveReason || "-"}</div>
      ),
    },
    {
      key: "action",
