...
const apiResultDriversList = {
  Message: {
    Code: 200,
    Text: "Available drivers retrieved successfully",
  },
  Data: {
    drivers: [
      {
...
export const fetcherDriversList = async (_, params = {}) => {
  ...
  if (isMockDriversList) {
    ...
    if (params.search) {
      ...
      const filteredDrivers = result.Data.drivers.filter(
        (driver) =>
  ...
  const result = await fetcherMuatrans.get("v1/drivers/available", {
    params: {
      page,
      limit,
      search,
